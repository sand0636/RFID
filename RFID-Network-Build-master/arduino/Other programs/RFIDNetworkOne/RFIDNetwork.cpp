...
void RFIDNetwork::update() {
  if(sleeping) {
    if(sleepTill-5 <= millis()) {
      awake();
    }
    else {
      return ;
  ...
  while (network.available()) {
    ...
    switch(header.type) {
      case 'R':
        ...
        break;
      case 'S':
        Serial.print("Sleep code recieved, radio sleeping for ");
        _t = 0;
        for(int i=0; i<10; i++) {
  ...
  if((retryTime < millis()-lastMillis) && numberOfReads > 0) {
    ...
    if(!mesh.write(&message, 'R', sizeof(message))) {
      if ( ! mesh.checkConnection() ) {
        //refresh the network address
        Serial.println("Renewing Address");
...
void RFIDNetwork::newRead(char *_rfidTag, long _t) {
  ...
  if(numberOfReads < readsCacheSize) {
    ...
    reads[numberOfReads][20] = ((readNumber++)%25)+33;  //The counter that prevents from duplicate reads beeing added
...
void RFIDNetwork::sleep() {
  ...
  */
  ...
  mesh.releaseAddress();
  radio.powerDown();
  Serial.println("Radio is turning off");
}

void RFIDNetwork::awake() {
  sleeping = false;
  radio.powerUp();
  mesh.renewAddress();
  Serial.println("Radio is turning on");
}
