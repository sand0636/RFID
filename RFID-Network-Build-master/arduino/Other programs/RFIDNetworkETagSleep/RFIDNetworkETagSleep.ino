 ...
 * Last change 6/24/2018
 * Fixed bug saving to many "BOARD STILL ON"
 */
...
String readerID = "Test"; //The reader id; can be alphanumeric; add leading zeros if you want them
...
byte slpM = 01;                            //When to go to sleep at night - minute
byte wakH = 23;                            //When to wake up in the morning - hour             
byte wakM = 05;                            //When to wake up in the morning - minute 
    ...
    if (RFcircuit == 1) { //Determin which RFID circuit to activate
      ...
      while (stopMillis > millis()) {          //As long as the stoptime is less than the current millisecond counter, then keep looking for a tag
          ...
          if(gManDecoder1.DecodeAvailableData(&xd) > 0) {   
            ...
            // Add read to RFIDNetwork
            char tbuf[10];
            ManchesterDecoder::GetHexString(&xd,tbuf,sizeof(tbuf));
            serial.println(tbuf);
            
            RFIDNetwork.newRead(tbuf, rtc.now().unixtime());
          } // end ScanForTag
      }
      gManDecoder1.DisableMonitoring();
    }  
...
void goToSleep() {
  ...
  // Adjust the RTCZero
  DateTime now = rtc.now();
  rtc0.setTime(now.hour(), now.minute(), now.second());
  rtc0.setDate(now.day(), now.month(), now.year());
